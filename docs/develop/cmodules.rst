...
Tier 1 & 2 ports have varying support for C dynamic memory allocation:
...
It's strongly recommended to follow the pattern demonstrated
in``examples/usercmodule/cppexample``, where the Python module is implemented in
a minimal C file wrapper around the C++ code.
