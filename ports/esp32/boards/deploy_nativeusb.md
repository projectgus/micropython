...
```bash
esptool.py write_flash {flash_offset} ESP32_BOARD_NAME-DATE-VERSION.bin
```
...
```bash
esptool.py --port PORTNAME write_flash {flash_offset} ESP32_BOARD_NAME-DATE-VERSION.bin
```

### Troubleshooting

For more information, consult the [esptool
